...
var PLib = 
	{
		...
		CustomDaynum: function(d) {
			return (d - 315446400000) / 86400000;
		},
		...
		Calc: function()
		{
			...
			PLib.sat_alt = parseFloat(sat_geodetic.alt);
			...
			/* No, we don't want rounding - rumith */
		...
		QuickFind: function(satname)
		{
			...
			{
				if ((PLib.sat[z].name == satname) || (parseInt(satname, 10) == parseInt(PLib.sat[z].catnum, 10)))
				{
					...
					if (PLib.Decayed(z, PLib.daynum) == 0)
					{
						...
						if (satInfo.latitude > 85) {
							satInfo.latitude = 85;
						}
						else if (satInfo.latitude < -85) {
							satInfo.latitude = -85;
						}
		...
		CustomFind: function(satname, dt)
		{
			var satInfo = new Object();
			for (var z = 0; z < PLib.sat.length; z++)
			{
				if ((PLib.sat[z].name == satname) || (parseInt(satname, 10) == parseInt(PLib.sat[z].catnum, 10)))
				{
					PLib.daynum = PLib.CustomDaynum(dt);
					PLib.PreCalc(z);
					PLib.Calc();
